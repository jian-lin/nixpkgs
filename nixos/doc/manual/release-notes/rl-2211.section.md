...
- `emacs` enables native compilation and disables asynchronously deferred native compilation by setting `native-comp-deferred-compilation` to `nil`, which means:
  - emacs packages from nixpkgs, builtin or not, do native compilation ahead of time so you can enjoy the benefit of native compilation without compiling them on you machine;
  - emacs packages from somewhere else, e.g. `package-install`, will not do any native compilation unless you set `native-comp-deferred-compilation` to `t`.
